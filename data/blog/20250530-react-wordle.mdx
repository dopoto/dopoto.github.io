...
So, _a few hours later_, here's my attempt at it - yet another React Wordle implementation:
...
Since there are no actual input fields on the screen that would trigger it, there's no keyboard
popping up on mobile devices. So, before rendering the grid, we'll need to do some hacking to force it to show up:
