...
Just watched the live coding exercise above - about implementing Wordle in React in 1 hour.

They missed an edge-but-no-so-edge-really case: handling multiple matches
...
Not sure I would've handled all of this better in a live-coding, time-caped situation, but still - there's a lot of things
I would've approached in a different way. So, you know how it goes, proof is in the pudding and all that - 2.5 hours later,
...
At any point during the game, the grid that gets rendered consists of three sections:

- your accepted guesses - checked against the solution. These go in the top rows.
- the current guess - always a single row, 1 to 5 letters, not accepted yet / not checked against the solution yet.
- your remaining attempts - these are just blank rows

So, when rendering the grid, your accepted guesses and your current guess should be taken from the internal
component state, where ideally they have already been stored in an easy-to-use format (compared against solution,
colors already assigned to each letter etc).
...
Now, we need to build the game rules parser - this kicks in whenever the player submits a new word. We'll place
this function outside of the component. It's the core part of the game, and we want to be able to easily
...
<Image alt="Wordle" src="/static/images/wordle.png" width={437} height={603} />
