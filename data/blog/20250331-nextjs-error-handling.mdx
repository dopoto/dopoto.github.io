...
Now, that struck me as a bit odd... Why not log errors directly in your server component? Why send them
to your client-side app? What if sensitive data that you don't want your users to see gets sent?

Well, although the docs don't mention it, React <a href="https://github.com/facebook/react/blob/b7e7f1a3fab87e8fc19e86a8088a9e0fe4710973/packages/react-server-dom-webpack/src/__tests__/ReactFlightDOMEdge-test.js#L980-L983">takes care of the sensitive part</a> - in your production bundle, all
errors sent this way get sanitized automatically - the actual error message gets replaced with this generic one:
...
Still - none of the above matches what I would actually need to happen in this case in an actual production app:

### On the server side:

- populate a custom error context, including potentially sensitive data
- generate a unique reference number for this error instance
